...
async function verifyWebhookOrders() {
  ...
  const webhookOrders = recentOrders.filter(o => o.stripePaymentIntentId || o.stripeSubscriptionId)
  const clientOrders = recentOrders.filter(o => !o.stripePaymentIntentId && !o.stripeSubscriptionId)
  ...
  const paymentIntentIds = webhookOrders
